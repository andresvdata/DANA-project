...
```{r}
hist(df_new_clean$Rating, main='Histogram for Rating',xlab = 'Rating')
```

# Question 4 

```{r}
plot(log(df_new_clean$Installs)~df_new_clean$Reviews)
...
df_new_paid_clean<-df_new_paid_clean[df_new_paid_clean$Price<40,]

boxplot(df_new_paid_clean$Price,horizontal = TRUE)
```
Better distribution. 

```{r}
df_new_paid_clean<-df_new_paid_clean[df_new_paid_clean$Reviews<500000,]

modelo <- lm (df_new_paid_clean$Reviews ~ log(df_new_paid_clean$Rating )
summary(modelo)
```

```{r}
plot(log(df$Reviews) ~ df$Installs  )
abline(modelo)
cor(df_new_paid_clean$Rating,df_new_paid_clean$Reviews)
```
...
```{r}
df_top<-df_new_clean[order(-df_new_clean$Price),][1:20,]
df_top
```
...
```{r}
df_top<-df_new[order(-df_new$Price),][1:20,]
df_top
```
...
```{r}
plot(df$Price~log(df$Reviews))
l<- lm(df$Price~df$Reviews)
abline(l)
```
...
```{r}
table(df_new$Category,df_new$Installs)
```
